...
class Redis
  ...
  class Set < BaseObject
    ...
    # Redis: SADD
    def add(value)
      redis_proxy.rcmd(:sadd, key, to_redis(value))
    end

    # Remove and return a random member.  Redis: SPOP
    def pop
      from_redis redis_proxy.rcmd(:spop, key)
    end

    # Adds the specified values to the set. Only works on redis > 2.4
    # Redis: SADD
    def merge(*values)
      redis_proxy.rcmd(:sadd, key, values.flatten.map{|v| to_redis(v)})
    end

    # Return all members in the set.  Redis: SMEMBERS
    def members
      v = from_redis redis_proxy.rcmd(:smembers, key)
      v.nil? ? [] : v
    end
    ...
    def member?(value)
      redis_proxy.rcmd(:sismember, key, to_redis(value))
    end
    alias_method :include?, :member?

    # Delete the value from the set.  Redis: SREM
    def delete(value)
      redis_proxy.rcmd(:srem, key, to_redis(value))
    end

    # Delete if matches block
    def delete_if(&block)
      res = false
      redis_proxy.rcmd(:smembers, key).each do |m|
        if block.call(from_redis(m))
          res = redis_proxy.rcmd(:srem, key, m)
        end
      end
      res
    end
    ...
    def intersection(*sets)
      from_redis redis_proxy.rcmd(:sinter, key, *keys_from_objects(sets))
    end
    ...
    def interstore(name, *sets)
      redis_proxy.rcmd(:sinterstore, name, key, *keys_from_objects(sets))
    end
    ...
    # Redis: SUNION
    def union(*sets)
      from_redis redis_proxy.rcmd(:sunion, key, *keys_from_objects(sets))
    end
    alias_method :|, :union
    alias_method :+, :union
    ...
    def unionstore(name, *sets)
      redis_proxy.rcmd(:sunionstore, name, key, *keys_from_objects(sets))
    end
    ...
    # Redis: SDIFF
    def difference(*sets)
      from_redis redis_proxy.rcmd(:sdiff, key, *keys_from_objects(sets))
    end
    ...
    def diffstore(name, *sets)
      redis_proxy.rcmd(:sdiffstore, name, key, *keys_from_objects(sets))
    end
    ...
    def move(value, destination)
      redis_proxy.rcmd(:smove, key, destination.is_a?(Redis::Set) ? destination.key : destination.to_s, value)
    end

    # The number of members in the set. Aliased as size. Redis: SCARD
    def length
      redis_proxy.rcmd(:scard, key)
    end
